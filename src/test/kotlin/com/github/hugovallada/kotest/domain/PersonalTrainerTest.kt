...
class PersonalTrainerTest : FeatureSpec({
	...
	feature("changePrice") {
		scenario("When the changePrice is called and it's bigger than 0, then the getCurrentPrice should return the new value") {
			checkAll(iterations = 50, Arb.int(1..10000)) {
				personal.changePrice(BigDecimal(it))
				personal.getCurrentPrice() shouldBe BigDecimal(it)
			}
		}
		scenario("When the changePrice is called and it's less than 0, then the getCurrentPrice should return the old price") {
	...
	feature("calendar") {
		scenario("When a event is added on calendar, then when the method see calendar is called, then should return the calendar") {
		...
		scenario("When an event is added on calendar for an client that already has one, then should return the calendar with the new time") {
			val time = ZonedDateTime.now().plusDays(2)
			personal.addNewEntryOnCalendar(client, time)
